...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            node_a = PgNode_a(action) # Building a node for action
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # let's go thru all the actions of node_a1 and see if anything colides with removed actions in node_a2
        for action  in node_a1.action.effect_add:
            if action in node_a2.action.effect_rem:
                return True

        # let's go thru all the actions of node_a2 and see if anything colides with removed actions in node_a1
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        b_result = node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos
        return b_result
    ...
    def h_levelsum(self) -> int:
        ...
        print_debug("Here is the goal: {}".format(self.problem.goal))
        store_level = -1
        for goal in self.problem.goal:
            b_found = False
            print_debug("Searching for goal: {}".format(goal))
            print_debug("We have {} levels".format(len(self.s_levels)))
            for level in range(len(self.s_levels)):
                if level <= store_level:
                    continue
                for state in self.s_levels[level]:
                    print_debug("Taking a look at state: {} on the level {}".format(state.symbol, level))
